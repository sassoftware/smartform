...
public class ValidationHelper extends LifecycleObject implements IValidationAware
{
    ...
    public function set isValid(value:Boolean):void
    {
        ...
        if (target)
        {
            ...
            //trace("VH isValid "+ _isValid + " with target " + target["id"]);
        }
        else
        {
            //trace("VH isValid "+ _isValid + " without target");
        }
    }
    ...
    private function handleTargetRemovedFromStage(event:Event):void
    {
        ...
        if (event.target == target)
        {
            suspendAllValidators();
        }
    }

    private function refreshItemsToValidate():void
    {
        suspendAllValidators();
        errorTipManager.reset();
        
        // re-add them all
        _setItemsToValidate(itemsToValidate);
    }
    ...
    public function suspendAllValidators():void
    {
        var v:*;
    ...
    public function resumeAllValidators():void
    {
        var v:*;
        
        for each (v in itemsToValidate)
        {
            resumeItemToValidate(v);
        }
    ...
    // TODO: change suspend/resume to use a simpler flag strategy on this
    // ValidationHelper that would enable/disable event reactions
    public function resumeItemToValidate(v:IEventDispatcher):void
    {
        var valAware:IValidationAware = v as IValidationAware;
        
        addItemToValidate(v);
        if (valAware && valAware.validationHelper)
        {
            valAware.validationHelper.resumeAllValidators();
        }
    }
    ...
    public function suspendItemToValidate(v:*):void
    {
        ...
        {
            if (valAware.validationHelper)
                valAware.validationHelper.suspendAllValidators();
        }
        else
        {
    ...
    public function set itemsToValidate(vals:Array):void
    {
        ...
        removeAllValidators();
        suspendAllValidators();
        _setItemsToValidate(vals);
    }
    
    private function _setItemsToValidate(vals:Array):void
    {
        var v:*;
        for each (v in vals)
        {
